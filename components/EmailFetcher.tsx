...
const EmailFetcher = ({ userProfile, refreshProfile }: EmailFetcherProps) => {
    ...
    useEffect(() => {
        ...
        let interval = setInterval(async () => {
            try {
                ...
                if (response.data.status.toUpperCase() === "FAILURE" || response.data.status.toUpperCase() === "SUCCESS") {
                    setIsFetching(false);
    ...
    const handleFetchJobs = async (skipFirstTimeCheck = false) => {
        ...
        if (!userProfile?.sheet_id) {
            toast.error("Please connect a Google Sheet first to fetch jobs from email.");
            return;
        }
