...
export function RouteGuard({
  ...
  useEffect(() => {
    ...
    switch (type) {
      case 'protected':
        if (!isAuthenticated) {
          console.log(
            'RouteGuard - Final check: User not authenticated, redirecting',
          )
          setIsRedirecting(true)
          router.replace(redirectTo || '/')
        } else {
      ...
      case 'public':
        if (isAuthenticated) {
          console.log(
            'RouteGuard - User authenticated, redirecting from public route',
          )
          setIsRedirecting(true)
          router.replace(redirectTo || '/dashboard')
        } else {
          console.log(
            'RouteGuard - User not authenticated, allowing public access',
          )
          setHasCheckedAuth(true)
        }
        break
  ...
  if (type === 'public' && isAuthenticated) {
    console.log(
      'RouteGuard - Final render check: Blocking public content for authenticated user',
    )
    return null
  }
...
export const AnyRoute = ({
  children,
  fallback,
}: Omit<RouteGuardProps, 'type' | 'redirectTo'>) => (
